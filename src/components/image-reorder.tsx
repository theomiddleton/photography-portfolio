  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Drag and drop images to reorder them manually, or use automatic
          colour-based sorting. Warning: Due to being a masonry layout, the
          order may not be be what you expect
        </CardDescription>
      </CardHeader>
      <CardContent>
        <DndContext
        ...
        >
          <Table>
            ...
            <TableBody>
              <SortableContext
                items={images.map((img) => img.order)}
        ...
        <div className="mb-4 flex gap-4">
          ...
          {updateStatus && (
            <p
              className={`text-sm ${updateStatus.includes('success') ? 'text-green-600' : 'text-red-600'}`}
            >
              {updateStatus}
            </p>
          )}
      ...
      <CardFooter className="flex flex-col items-start">
        <div className="mb-4 text-xs text-muted-foreground">
          Showing <strong>1-{images.length}</strong> of{' '}
        ...
        {showPreview && (
          <div className="mt-4 w-full">
            ...
            <ImageLayoutPreview
              images={showHidden ? images : images.filter((img) => img.visible)}
            />
          </div>
        )}
      </CardFooter>
