...
interface SigninFormProps {
  login: (prevState: LoginState, data: FormData) => Promise<LoginState>
}
...
export function SigninForm({ login }: SigninFormProps) {
  ...
  // Animate card on error (only on subsequent errors, not the first one)
  useEffect(() => {
    if (state.message && state.message !== prevMessageRef.current) {
      // Only animate if there was a previous error (not the first error)
      if (prevMessageRef.current !== '' && cardRef.current) {
      ...
      // Update the ref to track this message
