...
export function EnhancedProductView({ product, sizes, recommendations = [] }: EnhancedProductViewProps) {
  ...
  const totalPrice = selectedPrintSize ? selectedPrintSize.totalPrice * quantity : 0
      ...
      <div className="grid lg:grid-cols-2 gap-12 mb-16">
          ...
          <Dialog open={showZoom} onOpenChange={setShowZoom}>
            <DialogTrigger asChild>
              <div className="relative cursor-zoom-in group">
                ...
                <Tabs value={currentView} onValueChange={(v) => setCurrentView(v as 'image' | 'wall')} className="w-full">
                  <TabsList className="absolute top-4 left-4 z-10 bg-white/90">
                    <TabsTrigger value="image" className="text-xs">Photo</TabsTrigger>
                    <TabsTrigger value="wall" className="text-xs">On Wall</TabsTrigger>
                  </TabsList>
                  ...
                  <TabsContent value="wall" className="mt-0">
                    <div className="relative w-full aspect-square rounded-xl overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-1/3 -translate-y-4 -translate-x-4">
                          <Frame
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className={cn(isWishlisted && "bg-red-50 border-red-200")}
                >
            ...
            {!product.active && (
              <Badge variant="destructive" className="mb-4">Currently Unavailable</Badge>
            )}
          </div>
                  ...
                  {sizes.map((size) => (
                    <SelectItem key={size.id} value={size.id}>
                      <div className="flex justify-between items-center w-full">
                        <span>{size.name}</span>
                        <span className="ml-4 font-medium">{formatPrice(size.totalPrice)}</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <div className="space-y-3">
            <Button 
              ...
              size="lg"
            >
              {loading ? 'Processing...' : `Add to Cart - ${formatPrice(totalPrice)}`}
            </Button>
