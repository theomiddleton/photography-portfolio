...
const PREDEFINED_DETAILS = [
  ...
  {
    label: 'Mounting',
    value: 'Professional mounting available',
    category: 'construction',
  },
  {
    label: 'Paper Weight',
    value: '250gsm professional grade',
    category: 'construction',
  },
  { label: 'Color Profile', value: 'sRGB color space', category: 'appearance' },
  {
    label: 'Archival Quality',
    value: 'Museum-grade archival inks',
    category: 'quality',
  },
  {
    label: 'Longevity',
    value: '100+ year fade resistance',
    category: 'quality',
  },
  { label: 'Border', value: 'White border optional', category: 'appearance' },
  {
    label: 'Signature',
    value: 'Artist signature available',
  ...
  },
  {
    ...
    value: 'Numbered limited prints',
    category: 'exclusivity',
  },
  {
    label: 'Size Options',
    value: 'Multiple size formats',
    category: 'customization',
  },
  {
    label: 'Gift Wrapping',
    value: 'Professional gift presentation',
    category: 'service',
  },
]
...
export function ProductDetailManagement({
  ...
  const [editingDetail, setEditingDetail] = useState<StoreProductDetail | null>(
    null,
  )
  const [activeTab, setActiveTab] = useState('global')
  const [selectedProduct, setSelectedProduct] = useState<string>('')
  const [selectedCategory, setSelectedCategory] = useState<string>('')
  const [draggedItem, setDraggedItem] = useState<StoreProductDetail | null>(
    null,
  )
  ...
  const handleUpdateDetail = async (
    detailId: string,
    updates: Partial<StoreProductDetail>,
  ) => {
    try {
  ...
  const handlePredefinedDetailSelect = (
    predefined: (typeof PREDEFINED_DETAILS)[0],
  ) => {
    setNewDetail((prev) => ({
      ...prev,
  ...
  const handleBulkApplyDetails = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(
          `Applied ${result.count} global details to product successfully`,
        )
      } else {
  ...
  return (
    <Card className={className}>
      <CardHeader>
                    ...
                    <div className="grid max-h-60 grid-cols-1 gap-2 overflow-y-auto">
                      {filteredPredefinedDetails.map((predefined, index) => (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            handlePredefinedDetailSelect(predefined)
                          }
                ...
                <div className="space-y-4 border-t pt-4">
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setNewDetail((prev) => ({ ...prev, isGlobal: checked }))
                      }
                    />
                    <Label htmlFor="detail-global">
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setNewDetail((prev) => ({ ...prev, active: checked }))
                      }
                    />
        ...
        <CardDescription>
          Manage product details shown on individual product pages. Global
          details apply to all products.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="mt-2 space-y-2">
                {globalDetails.length === 0 ? (
                  <div className="py-8 text-center text-gray-500">
                    No global details created yet. Add your first detail to get
                    started.
                  </div>
                ) : (
                    ...
                    <div
                      ...
                      onDrop={() => {
                        if (draggedItem && draggedItem.isGlobal) {
                          const dragIndex = globalDetails.findIndex(
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            handleUpdateDetail(detail.id, { active: checked })
                          }
                        />
                        <Button
                          variant="ghost"
          ...
          <TabsContent value="products" className="space-y-4">
            <div className="py-8 text-center text-gray-500">
              Product-specific details coming soon. Currently showing global
          ...
          <TabsContent value="bulk" className="space-y-4">
            <div>
              <Label className="text-sm font-medium">
                Apply Global Details to Product
              </Label>
              <div className="mt-2 space-y-4">
                ...
                <Button
                  onClick={handleBulkApplyDetails}
                  disabled={!selectedProduct}
                >
                  Apply Global Details to Selected Product
                </Button>
                <div className="text-sm text-gray-600">
                  This will copy all active global details to the selected
                  product as product-specific details.
                </div>
              </div>
            </div>
          </TabsContent>
