...
import { AlertCircleIcon, ImageIcon, UploadIcon, XIcon, Sparkles, Loader2, Trash2, StopCircle } from 'lucide-react'
...
const defaultPrintSize: PrintSize = { name: '8x10', width: 8, height: 10, basePrice: 2500 }

export function BatchUpload({ bucket, draftId, onImageUpload }: BatchUploadProps) {
  const maxSizeMB = 20
  ...
  const generateAIForImage = async (imageId: string, field?: 'title' | 'description' | 'tags') => {
    const image = batchImages.find(img => img.id === imageId)
    if (!image?.url) {
    ...
    try {
      const tasks = image.aiGenerated && field ? [field] : ['title', 'description', 'tags']
      const metadata = await generate(image.url, tasks)

      setBatchImages(prev => prev.map(img => {
        if (img.id === imageId) {
          const updates: Partial<BatchImageData> = { aiGenerated: true }
          if (metadata.title && tasks.includes('title')) updates.name = metadata.title
          if (metadata.description && tasks.includes('description')) updates.description = metadata.description
          if (metadata.tags && tasks.includes('tags')) updates.tags = metadata.tags
          return { ...img, ...updates }
        }
        return img
      }))
  ...
  const updateImageData = (imageId: string, field: keyof BatchImageData, value: any) => {
    setBatchImages(prev => prev.map(img => 
      img.id === imageId ? { ...img, [field]: value } : img
    ))
  }

  const updatePrintSize = (imageId: string, sizeIndex: number, field: keyof PrintSize, value: string | number) => {
    setBatchImages(prev => prev.map(img => {
      if (img.id === imageId) {
  ...
  const uploadAllImages = async () => {
    ...
    const uploadTasks = imagesToUpload.map(async (image) => {
      ...
      try {
        ...
        const controller = new AbortController()
        setBatchImages(prev => prev.map(img => 
          img.id === image.id ? { ...img, uploadController: controller } : img
        ))
          ...
          return new Promise<void>((resolve) => {
            ...
            xhr.upload.onprogress = (event) => {
              if (event.lengthComputable) {
                ...
                setUploadProgress(prev => ({ ...prev, [image.id]: percentComplete }))
              }
            }
            ...
            xhr.onload = () => {
              if (xhr.status === 200) {
                setBatchImages(prev => prev.map(img => 
                  img.id === image.id 
                    ? { ...img, uploading: false, uploaded: true, url: fileUrl }
                    : img
                ))
                if (onImageUpload) {
  ...
  const cancelAllUploads = async () => {
    const uploadingImages = batchImages.filter(img => img.uploading && img.uploadController)
          ...
          {batchImages.length > 0 && (
            <div className="flex gap-2">
              <Button
                ...
                disabled={aiLoading || batchImages.every(img => img.aiGenerated || !img.url)}
              >
                {aiLoading ? (
      ...
      <CardContent className="space-y-6">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="mb-1.5 text-sm font-medium">Drop multiple images here</p>
            <p className="text-muted-foreground text-xs mb-2">
              SVG, PNG, JPG or GIF (max. {maxSizeMB}MB each, up to {maxFiles} files)
            </p>
        ...
        {batchImages.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">{batchImages.length} Images Ready</h3>
              <Button
                onClick={uploadAllImages}
                disabled={isUploading || batchImages.every(img => img.uploaded)}
                className="min-w-32"
              >
            ...
            <div className="grid gap-4">
              {batchImages.map((image) => (
                <Card key={image.id} className={`${image.uploaded ? 'bg-green-50 dark:bg-green-950/20' : ''}`}>
                      ...
                      <div className="col-span-9 grid gap-4">
                        <div className="grid grid-cols-12 gap-2 items-center">
                          ...
                          <div className="col-span-9">
                            <Input
                              value={image.name}
                              onChange={(e) => updateImageData(image.id, 'name', e.target.value)}
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => generateAIForImage(image.id, 'title')}
                            disabled={aiLoading || !image.url || image.uploaded}
                          ...
                          <>
                            <div className="grid grid-cols-12 gap-2 items-start">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => generateAIForImage(image.id, 'description')}
                                disabled={aiLoading || !image.url || image.uploaded}
                                className="col-span-1 h-8 px-2"
                              >
                                <Sparkles className="h-3 w-3" />
                            ...
                            <div className="grid grid-cols-12 gap-2 items-center">
                              ...
                              <div className="col-span-9">
                                <Input
                                  value={image.tags}
                                  onChange={(e) => updateImageData(image.id, 'tags', e.target.value)}
                                  placeholder="Tags"
                                  disabled={image.uploading || image.uploaded}
                                />
                              </div>
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => generateAIForImage(image.id, 'tags')}
                                disabled={aiLoading || !image.url || image.uploaded}
                                className="col-span-1 h-8 px-2"
                              >
                                <Sparkles className="h-3 w-3" />
                            ...
                            <div className="grid grid-cols-12 gap-2 items-center">
                              <Label className="col-span-2 text-sm">For Sale</Label>
                              <div className="col-span-10">
                                <input
                                  type="checkbox"
                                  checked={image.isSale}
                                  onChange={(e) => updateImageData(image.id, 'isSale', e.target.checked)}
                                  disabled={image.uploading || image.uploaded}
                                  className="size-4"
                                />
                              </div>
                            ...
                            {image.isSale && (
                              <div className="col-span-12 space-y-2">
                                ...
                                {image.printSizes.map((size, sizeIndex) => (
                                  <div key={sizeIndex} className="grid grid-cols-4 gap-2">
                                    <Input
                                      value={size.name}
                                      onChange={(e) => updatePrintSize(image.id, sizeIndex, 'name', e.target.value)}
                                    ...
                                    <Input
                                      type="number"
                                      value={size.width}
                                      onChange={(e) => updatePrintSize(image.id, sizeIndex, 'width', Number(e.target.value))}
                                      placeholder="Width"
                                      disabled={image.uploading || image.uploaded}
                                    />
                                    <Input
                                      type="number"
                                      value={size.height}
                                      onChange={(e) => updatePrintSize(image.id, sizeIndex, 'height', Number(e.target.value))}
                                      placeholder="Height"
                                      disabled={image.uploading || image.uploaded}
                                    />
                                    <Input
                                      ...
                                      onChange={(e) => updatePrintSize(image.id, sizeIndex, 'basePrice', e.target.value)}
                                      placeholder="Price (£)"
                                      disabled={image.uploading || image.uploaded}
                                    />
                                  </div>
