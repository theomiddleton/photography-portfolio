...
export const components = {
  ...
  ImageViewer: ({ images }: { images: (string | { src: string, alt?: string, width?: number, height?: number })[] }) => {
    if (!Array.isArray(images)) {
      console.warn('ImageViewer: images prop must be an array')
      return null
    }
    ...
    const viewerImages = images.map(image => {
      ...
      return {
        src: image.src,
        alt: image.alt || 'Image Viewer Image',
