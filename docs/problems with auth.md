...
/////////// /I then found that the `bcrypt` library was not compatible with entirely Next.js, and that I should use `bcryptjs` instead.

The `bcrypt` library is vital for hashing passwords, but when reading the documentation, I discovered it was written in C++, making it incompatible with Next.js due to the slightly different environment of the browser and server.

for debugging, i separated the functions into seperate files, trying to avoid mixing bcrypt and jose in the same file as much as possible.
